...
export class MechProductComponentsService {
  ...
  constructor(
    @InjectRepository(MechProductComponent) private repository: Repository<MechProductComponent>,
  ...
  async installComponent(productId: number, holderId: number, componentId: number) {
    return this.create(new CreateMechProductComponentDto(productId, holderId, componentId));
  }

  @ApiOperation({ description: 'Proxy method, see Create(...) for details' })
  async changeComponent(productId: number, componentId: number, newComponentId: number): Promise<MechProductComponent> {
    ...
    try {
      ...
      let res = await this.createTransactional(queryRunner, new CreateMechProductComponentDto(productId, mpc?.holder?.id, newComponentId));
  ...
  @ApiOperation({ description: "Create linking three entities object: Product, Holder component and Child component" })
  async createTransactional(queryRunner: QueryRunner, dto: CreateMechProductComponentDto): Promise<MechProductComponent> {

    try {

      const e = await this.loadDtoAsObject(dto);

      let res = await queryRunner.manager.save(MechProductComponent, e);
  ...
  @ApiOperation({ description: "Create linking three entities object: Product, Holder component and Child component" })
  async create(dto: CreateMechProductComponentDto): Promise<MechProductComponent> {

    const e = await this.loadDtoAsObject(dto);
    let res = await this.repository.save(e);

    return res;
  }

  async loadDtoAsObject(dto: CreateMechProductComponentDto): Promise<MechProductComponent> {
    const e = new MechProductComponent();

    if (dto.productId) {
      e.product = await this.productService.findOne(dto.productId);
      assertNotNullUndefined(e.product, `Product with id=${dto.productId} not found`);
    }

    e.component = await this.componentService.findOne(dto.componentId);
    assertNotNullUndefined(e.component, `Component with id=${dto.componentId} not found`);

    // Holder component might be empty for a top level Component
    if (dto.holderId) {
      e.holder = await this.componentService.findOne(dto.holderId);
      assertNotNullUndefined(e.holder, `Component with id=${dto.holderId} not found to be used as holder`);

      if (e.holder.id === e.component.id) {
        throw new MechException(`HolderId=${e.holder.id} and ComponentId=${e.component.id}. Prohibited to install self-cyclic component links`)
      }
    }
    ...
    if (links && links.length > 0) {
      let linkListStr = links.map(x => (`(ProductId=${x.product?.id || '<null>'}, HolderId=${x.holder?.id || '<null>'})`)).join(",");

      throw new MechException(`ComponentId=${e.component.id} already installed into ` + linkListStr);
    }
    ...
    while (xHolderId && xHolderId !== undefined) {
      let he = links2.find((e) => e.component?.id === xHolderId);
      if (he && he !== undefined) {
    ...
    if (cycleAttemptFound) {
      throw new MechException(`Circular dependency attempt detected, not allowed`);
    }
  ...
  productComponentsToComponentTree(array: MechProductComponent[]): MechComponent[] {
